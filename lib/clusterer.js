...
const clusterer = (() => {
  ...
  const GARBAGE_WORDS = new Set([
    "error", "bad", "gateway", "found", "forbidden", "unauthorized",
    "internal", "server", "timeout", "undefined", "null", "nan",
    "404", "500", "502", "503", "301", "302", "400", "401", "403",
    "loading", "untitled", "document", "welcome", "site", "website",
  ]);
  ...
  function clusterTabs(allTabData, parentChainFn, threshold = 0.25) {
    ...
    // Name each cluster
    const named = clusters.map((clusterTabIds) => {
      // Try TF-IDF terms first, filtering garbage
      const merged = {};
      for (const id of clusterTabIds) {
        for (const [term, score] of Object.entries(vectors[id] || {})) {
          if (!GARBAGE_WORDS.has(term) && !/^\d+$/.test(term) && term.length > 2) {
      ...
      if (topTerms.length > 0) {
        name = topTerms.map((t) => t.charAt(0).toUpperCase() + t.slice(1)).join(" / ");
      } else {
        // Fallback: most common domain
        const domains = {};
        for (const id of clusterTabIds) {
          try {
            const d = new URL(allTabData[id].url).hostname.replace("www.", "");
            domains[d] = (domains[d] || 0) + 1;
          } catch {}
        }
        const topDomain = Object.entries(domains).sort((a, b) => b[1] - a[1])[0];
        name = topDomain ? topDomain[0] : "Other";
      }

      return { name, tabIds: clusterTabIds };
    });

    // Merge single-tab groups into "Other"
    const multiTab = named.filter((c) => c.tabIds.length > 1);
    const singles = named.filter((c) => c.tabIds.length === 1).flatMap((c) => c.tabIds);

    if (singles.length > 0) {
      multiTab.push({ name: "Other", tabIds: singles });
    }
